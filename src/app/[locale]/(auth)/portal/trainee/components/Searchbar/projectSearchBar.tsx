...
import { selectAvailableJobs } from "@/app/[locale]/(auth)/portal/trainee/Slice/jobs/jobsSelectors";
const { useBreakpoint } = Grid;
import { useSelector } from "react-redux";

interface AvailableProjectSearchBarProps {
  placeholder?: string;
  label?: string;
}

export default function AvailableProjectSearchBar({
  placeholder = "Rechercher...",
  label,
}: AvailableProjectSearchBarProps) {
  const screens = useBreakpoint();
  const t = useI18n();
  // Fetches available jobs array from Redux
    ...
    >
      ...
      <div
        style={{
          ...
          maxWidth: screens.md ? "calc(100% - 140px)" : "none", // Adjusts width dynamically
        }}
      >
        <Input
        ...
        >
          <span>
            {/* Shows job count or fallback message based on available jobs */}
            {jobs.length > 0
              ? `${jobs.length} ${label || t("TraineeHome.Available")}`
              : t("TraineeJob.NoAvailable")}
          </span>
        </p>
      </div>
    </div>
